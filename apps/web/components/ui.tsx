...
export function Badge({ status, className }: BadgeProps) {
  ...
  const labels: Record<string, string> = {
    ...
    PENDING_OTP: "Awaiting Verification",
    COMPLETED: "Completed",
  };
...
export function EmptyState({ title, message, action, icon, details }: EmptyStateProps) {
  return (
    <div className="flex items-center justify-center min-h-[400px] px-4">
      <div className="text-center max-w-md">
        ...
        {/* Details */}
        {details && <p className="text-slate-500 text-xs mb-6 leading-relaxed">{details}</p>}
